...
export default config({
  ...
  collections: {
    ...
    // Sermon transcripts → content/transcripts/*.mdx
    // One per sermon, SAME slug as the sermon file — generated by
    // speech-to-text, editable here to fix names and mis-hearings.
    transcripts: collection({
      label: "Sermon Transcripts",
      ...
      schema: {
        title: fields.slug({
          name: {
            label: "Sermon (must match the sermon's slug)",
            description: "Use the exact same name as the sermon so they link up",
          },
        }),
        text: fields.mdx({ label: "Transcript" }),
      },
    }),
